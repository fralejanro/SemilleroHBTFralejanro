...
public class GestionarComicRest {
	...
	public ResultadoDTO crearComic(ComicDTO comicDTO) {
		ResultadoDTO resultadoDTO = new ResultadoDTO(Boolean.FALSE, "El comic no fue creado");
		if (gestionarComicEJB.crearComic(comicDTO) == Boolean.TRUE) {
	...
	public ResultadoDTO eliminarComic(@QueryParam("idComic") String idComic) {
		ResultadoDTO resultadoDTO = new ResultadoDTO(Boolean.FALSE, "El comic con id "+ idComic + " no existe");
		if (idComic != null) {
			if(gestionarComicEJB.eliminarComic(Long.parseLong(idComic))==Boolean.TRUE) {
				resultadoDTO = new ResultadoDTO(Boolean.TRUE, "Comic eliminado correctamente");
			}else{
				resultadoDTO = new ResultadoDTO(Boolean.FALSE, "El comic no fue eliminado");
			}
		}else {
			resultadoDTO = new ResultadoDTO(Boolean.FALSE, "El comic con"+ idComic + " no existe");
		}
		return resultadoDTO;
	}
}
